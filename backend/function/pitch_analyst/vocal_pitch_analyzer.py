...
def analyze_vocals(json_file, audio_file, output_file=None, log_file=None, verbose=True):
    """
    Phân tích pitch từ file giọng hát và gán note cho từng từ trong file JSON

    Parameters:
    -----------
    json_file : str
        Đường dẫn tới file JSON chứa lời bài hát đã được căn chỉnh
    audio_file : str
        Đường dẫn tới file âm thanh giọng hát (vocals)
    output_file : str, optional
        Đường dẫn để lưu file JSON kết quả. Nếu không cung cấp, sẽ tạo tên dựa trên json_file
    log_file : str, optional
        Đường dẫn để lưu file log chi tiết. Nếu không cung cấp, sẽ tạo tên dựa trên output_file
    verbose : bool, default=True
        Hiển thị thông báo tiến trình hay không

    Returns:
    --------
    dict
        Dữ liệu JSON đã được thêm thông tin về note
    """
    if verbose:
        print("Bắt đầu phân tích pitch từ file vocals...")
    start_time = time.time()
    
    # Tạo tên file output nếu không cung cấp
    if output_file is None:
        base_name = os.path.splitext(json_file)[0]
        output_file = f"{base_name}_with_notes.json"
    
    # Tạo tên file log nếu không cung cấp
    if log_file is None:
        base_output = os.path.splitext(output_file)[0]
        log_file = f"{base_output}_log.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Phân tích pitch từ file vocals và gán note vào file JSON')
    parser.add_argument('json_file', help='Đường dẫn tới file JSON chứa lời bài hát đã được căn chỉnh')
    parser.add_argument('audio_file', help='Đường dẫn tới file âm thanh giọng hát (vocals)')
    parser.add_argument('--output', '-o', help='Đường dẫn để lưu file JSON kết quả')
    parser.add_argument('--log', '-l', help='Đường dẫn để lưu file log chi tiết')
    parser.add_argument('--quiet', '-q', action='store_true', help='Không hiển thị thông báo tiến trình')
